...
namespace TennisVlaanderen_DAL.repositories
{
    public class TerreinReservatieRepository : BaseRepository, ITerreinReservatieRepository
    {
        ...
        {
            string sql = $"SELECT DISTINCT TerreinNummer, TypeOndergrond, Id, TypeTennis FROM TennisVlaanderen.TerreinReservatie T WHERE T.TypeOndergrond LIKE '%{TypeOndergrond}%'";
            using (IDbConnection db = new SqlConnection(ConnectionString))
            {
        ...
        public List<TerreinReservatie> OphalenTerreinId(int id)
        {
            string sql = $@"SELECT Id 
                           FROM TennisVlaanderen.TerreinReservatie
                           WHERE Id = {id}";
