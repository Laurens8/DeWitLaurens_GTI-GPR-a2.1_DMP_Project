...
namespace TennisVlaanderen_DAL.repositories
{
    public class SpelerClubTornooiRepository : BaseRepository, ISpelerClubTornooiRepository
    {
        ...
        public List<SpelerClubTornooi> OphalenTornooi(int id)
        {
            string sql = $"SELECT Id FROM TennisVlaanderen.SpelerClubTornooi WHERE Id = {id}";

            using (IDbConnection db = new SqlConnection(ConnectionString))
            {
                return db.Query<SpelerClubTornooi>(sql).ToList();
            }
        }
